...
const variableTypes = [
    ...
    },
    {
        ...
        name: 'entity',
        description: 'Variable will reference data from an entity'
    }
]
...
const AddEditVariableDialog = ({ show, dialogProps, onCancel, onConfirm, setError }) => {
    ...
    useEffect(() => {
        if (dialogProps.type === 'EDIT' && dialogProps.data) {
            ...
            setEntityId(dialogProps.data.entityId || '')
            setDialogType('EDIT')
    ...
    useEffect(() => {
        const loadEntities = async () => {
            try {
                ...
                console.log('Loaded entities:', response.data)
                if (response.data) {
    ...
    const addNewVariable = async () => {
        try {
            const obj = {
                ...
                value: variableValue,
                type: variableType,
                entityId: variableType === 'entity' ? entityId : null
            }
            ...
            <DialogContent>
                ...
                {variableType === 'entity' && (
                    <Box sx={{ p: 2 }}>
                        <div style={{ display: 'flex', flexDirection: 'row' }}>
                            <Typography>
                                Select Entity<span style={{ color: 'red' }}>&nbsp;*</span>
                            </Typography>
                            <div style={{ flexGrow: 1 }}></div>
                        </div>
                        {entities.length === 0 ? (
                            <Box sx={{ mt: 1, p: 2, bgcolor: 'warning.light', borderRadius: 1 }}>
                                <Typography variant="body2" color="warning.dark">
                                    No entities available. Please create an entity first from the Variables page.
                                </Typography>
                            </Box>
                        ) : (
                            <Dropdown
                                ...
                                onSelect={(newValue) => setEntityId(newValue)}
                                value={entityId ?? 'choose an entity'}
                                id='dropdown_entityId'
                            />
                        )}
                    </Box>
                )}
            ...
            <DialogActions>
                <StyledButton
                    disabled={
                        !variableName || 
                        !variableType || 
                        (variableType === 'static' && !variableValue) ||
                        (variableType === 'entity' && !entityId)
                    }
                    variant='contained'
