...
const getVariablesByEntityId = async (req: Request, res: Response, next: NextFunction) => {
    try {
        if (typeof req.params === 'undefined' || !req.params.entityId) {
            throw new InternalFlowiseError(StatusCodes.PRECONDITION_FAILED, 'Error: variablesController.getVariablesByEntityId - entityId not provided!')
        }
        const apiResponse = await variablesService.getVariablesByEntityId(req.params.entityId, req.user?.activeWorkspaceId)
        return res.json(apiResponse)
    } catch (error) {
        next(error)
    }
