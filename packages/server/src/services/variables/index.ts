...
const getVariablesByEntityId = async (entityId: string, workspaceId?: string) => {
    try {
        const appServer = getRunningExpressApp()
        const queryBuilder = appServer.AppDataSource.getRepository(Variable)
            .createQueryBuilder('variable')
            .where('variable.entityId = :entityId', { entityId })
            .orderBy('variable.updatedDate', 'DESC')
        ...
        throw new InternalFlowiseError(
            StatusCodes.INTERNAL_SERVER_ERROR,
            `Error: variablesServices.getVariablesByEntityId - ${getErrorMessage(error)}`
        )
    }
}
